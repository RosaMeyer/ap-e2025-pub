...
tests =
  testGroup
    "Checking"
    [
      -- checkExp $ CstInt 2 --> Nothing
      testCase "Example 1" $ checkExp (CstInt 2) 
      @?= Nothing,

      -- checkExp $ Var "x" --> Just "Variable not in scope: x"
      testCase "Example 2" $ checkExp (Var "x") 
      @?= Just "Variable not in scope: x",

      testCase "Testing CstBool" $ checkExp (CstBool True) 
      @?= Nothing
    ]
